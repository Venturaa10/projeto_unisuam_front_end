...
async function loadNavbar() { // Função para lidar com o carregamento de arquivos
    try {
        const response = await fetch('sections/sections_vendas.html'); // Busca o caminho do arquivo e armazena na const 
        const navbar = await response.text(); // Carrega o arquivo e converte para texto, para que seja manipulado
        document.getElementById('secao_vendas').innerHTML = navbar; // Inserindo o conteudo armazenado na const "navbar" e o substituindo no id="navbar"
